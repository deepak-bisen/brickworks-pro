 document.addEventListener('DOMContentLoaded', () => {
    ...
    // Simulation support: when true, mark-read operates locally without calling backend
    const SIMULATE_MARK_READ = true;
    // Keep last fetched messages in memory so we can re-render after local simulation
    let lastFetchedMessages = [];
    const simulatedReadIds = new Set();
    ...
    window.fetchAdminMessages = async function() {
        ...
        try {
            ...
            // Apply any simulated read flags
            lastFetchedMessages.forEach(m => {
                const id = m.id || m.messageId || m.contactId;
                if (simulatedReadIds.has(String(id))) m.status = 'READ';
            });
    ...
    async function deleteMessage(id) {
        try {
            const res = await fetch(`${API_URL}/api/v1/contact/${id}`, { method: 'DELETE', headers: getAuthHeader() });
    ...
    async function markMessageRead(id) {
        // If simulation is enabled, update local cache and re-render instead of calling backend
        if (typeof SIMULATE_MARK_READ !== 'undefined' && SIMULATE_MARK_READ) {
            try {
                simulatedReadIds.add(String(id));
                // Update cached messages
                lastFetchedMessages = lastFetchedMessages.map(m => {
                    const mid = m.id || m.messageId || m.contactId;
                ...
                renderAdminMessages(lastFetchedMessages);
                showStatus('Message marked as read (simulated).');
                return;
            } catch (err) {
                console.error(err);
                showStatus(err.message || 'Mark read failed', true);
                return;
            }
        }
